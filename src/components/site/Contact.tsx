    ...
    <section id="contact" className="py-20 lg:py-28 bg-secondary/50">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="text-center max-w-2xl mx-auto">
          <span className="text-sm font-semibold text-primary uppercase tracking-wider">Liên hệ</span>
...
function InfoCard({
...
}: { icon: React.ElementType; label: string; value: string; href?: string; highlight?: boolean }) {
  ...
  return (
    ...
    >
      <div className={`w-12 h-12 rounded-xl grid place-items-center mb-4 ${highlight ? "bg-white/20" : "bg-gradient-sun"}`}>
        <Icon className={`w-6 h-6 ${highlight ? "text-white" : "text-charcoal"}`} />
      </div>
      <div className={`text-xs uppercase tracking-wider font-semibold ${highlight ? "text-white/80" : "text-muted-foreground"}`}>
        {label}
      </div>
